/****************************************************************
  SIMPLE LoRa + WiFi Chat Node (with OLED status)
  - ESP32 AP, SSID set per node in WIFI_SSID
  - Open http://192.168.4.1 to chat
  - Browser asks user name (stored locally)
  - Messages sent over LoRa as "Name: message"
  - OLED shows SSID/IP at boot, LoRa status, and last RX
****************************************************************/
...
const char* WIFI_SSID  = "LoRaChat-Rakib"; // <- CHANGE THIS FOR EACH NODE
...
/*** helper: store message into ring buffer ***/
...
/*** helper: escape text for JSON string ***/
...
/*** LoRa send helper: send "Name: message" ***/
void loraSend(const String &name, const String &text) {
...
void setup() {
  ...
  delay(500);
  Serial.println();
  Serial.println("=== Simple LoRa Chat Node Boot ===");
